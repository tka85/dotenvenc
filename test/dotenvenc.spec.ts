...
import { DEFAULT_ENCRYPTED_FILE_READABLE } from '../src';

chai.use(chaiAsPromised);

// rewire resolves its target relative to module.parent, so it needs a real CommonJS
// require; an ESM import changes that resolution and breaks the lookup.
...
describe('encryption', () => {
    beforeEach(() => {
        ...
        writeFileSync(dotenvenc.DEFAULT_DECRYPTED_FILE, readFileSync(TEST_SAMPLE_DECRYPTED_FILE));
        writeFileSync(CUSTOM_DECRYPTED_FILE, readFileSync(TEST_SAMPLE_DECRYPTED_FILE));
    });
    ...
    it(`should encrypt default decrypted file ${dotenvenc.DEFAULT_DECRYPTED_FILE} into default encrypted file ${dotenvenc.DEFAULT_ENCRYPTED_FILE} without semi-readable '.readable' file generated`, async () => {
        ...
        expect(await dotenvenc.decrypt({ passwd: ENC_PASSWD, encryptedFile: dotenvenc.DEFAULT_ENCRYPTED_FILE })).to.deep.equal({ ALPHA: 'bar', BETA: 'foo bar', GAMMA: '1234', DELTA: 'With "double quotes" inside', DELTA_2: 'With \'single quotes\' inside', EPSILON: 'bla', KAPPA: 'multi\nline\nvalue' });
    });
    ...
    it(`should encrypt default decrypted file ${dotenvenc.DEFAULT_DECRYPTED_FILE} into default encrypted file ${dotenvenc.DEFAULT_ENCRYPTED_FILE} also with semi-readable '.readable' file generated`, async () => {
        ...
        writeFileSync(DEFAULT_ENCRYPTED_FILE_READABLE, readFileSync(TEST_SAMPLE_ENCRYPTED_FILE_READABLE));
        ...
        expect(await dotenvenc.decrypt({ passwd: ENC_PASSWD, encryptedFile: dotenvenc.DEFAULT_ENCRYPTED_FILE })).to.deep.equal({ ALPHA: 'bar', BETA: 'foo bar', GAMMA: '1234', DELTA: 'With "double quotes" inside', DELTA_2: 'With \'single quotes\' inside', EPSILON: 'bla', KAPPA: 'multi\nline\nvalue' });
        const encryptedFileReadableContents = readFileSync(DEFAULT_ENCRYPTED_FILE_READABLE, 'utf8');
        const encryptedFileReadableContentsReference = readFileSync(TEST_SAMPLE_ENCRYPTED_FILE_READABLE, 'utf8');
        expect(encryptedFileReadableContents).to.be.equal(encryptedFileReadableContentsReference);
    });
    ...
    it(`should encrypt default decrypted file ${dotenvenc.DEFAULT_DECRYPTED_FILE} into custom encrypted file ${CUSTOM_ENCRYPTED_FILE} without semi-readable '.readable' file generated`, async () => {
        ...
        expect(await dotenvenc.decrypt({ passwd: ENC_PASSWD, encryptedFile: CUSTOM_ENCRYPTED_FILE })).to.deep.equal({ ALPHA: 'bar', BETA: 'foo bar', GAMMA: '1234', DELTA: 'With "double quotes" inside', DELTA_2: 'With \'single quotes\' inside', EPSILON: 'bla', KAPPA: 'multi\nline\nvalue' });
    });
    ...
    it(`should encrypt default decrypted file ${dotenvenc.DEFAULT_DECRYPTED_FILE} into custom encrypted file ${CUSTOM_ENCRYPTED_FILE} also with semi-readable '.readable' file generated`, async () => {
        writeFileSync(CUSTOM_ENCRYPTED_FILE_READABLE, readFileSync(TEST_SAMPLE_ENCRYPTED_FILE_READABLE));
        await dotenvenc.encrypt({ passwd: ENC_PASSWD, encryptedFile: CUSTOM_ENCRYPTED_FILE, includeReadable: true });
        expect(await dotenvenc.decrypt({ passwd: ENC_PASSWD, encryptedFile: CUSTOM_ENCRYPTED_FILE })).to.deep.equal({ ALPHA: 'bar', BETA: 'foo bar', GAMMA: '1234', DELTA: 'With "double quotes" inside', DELTA_2: 'With \'single quotes\' inside', EPSILON: 'bla', KAPPA: 'multi\nline\nvalue' });
        const encryptedFileReadableContents = readFileSync(CUSTOM_ENCRYPTED_FILE_READABLE, 'utf8');
        const encryptedFileReadableContentsReference = readFileSync(TEST_SAMPLE_ENCRYPTED_FILE_READABLE, 'utf8');
        expect(encryptedFileReadableContents).to.be.equal(encryptedFileReadableContentsReference);
    });
    ...
    it(`should encrypt default decrypted file with DOTENVENC_PASS if empty password is provided and DOTENVENC_PASS is set`, async () => {
        ...
        await dotenvenc.encrypt({ passwd: '' });
        expect(await dotenvenc.decrypt({ passwd: process.env.DOTENVENC_PASS, encryptedFile: dotenvenc.DEFAULT_ENCRYPTED_FILE })).to.deep.equal({ ALPHA: 'bar', BETA: 'foo bar', GAMMA: '1234', DELTA: 'With "double quotes" inside', DELTA_2: 'With \'single quotes\' inside', EPSILON: 'bla', KAPPA: 'multi\nline\nvalue' });
    });
...
describe('decryption', () => {
    beforeEach(() => {
        ...
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, readFileSync(TEST_SAMPLE_ENCRYPTED_FILE));
        writeFileSync(CUSTOM_ENCRYPTED_FILE, readFileSync(TEST_SAMPLE_ENCRYPTED_FILE));
    });
    ...
    it(`should decrypt default encrypted file ${dotenvenc.DEFAULT_ENCRYPTED_FILE} correctly if explicitly passed password`, async () => {
        ...
        expect(data).to.deep.equal({ ALPHA: 'bar', BETA: 'foo bar', GAMMA: '1234', DELTA: 'With "double quotes" inside', DELTA_2: 'With \'single quotes\' inside', EPSILON: 'bla', KAPPA: 'multi\nline\nvalue' });
    ...
    it(`should reject a tampered ciphertext instead of returning altered secrets`, async () => {
        const [version, saltText, ivText, authTagText, encText] = readFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, 'utf8').split(':');
        ...
        ciphertext[0] ^= 0xff;
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, [version, saltText, ivText, authTagText, ciphertext.toString('hex')].join(':'));
    ...
    it(`should reject a tampered authentication tag`, async () => {
        ...
        authTag[0] ^= 0xff;
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, [version, saltText, ivText, authTag.toString('hex'), encText].join(':'));
        await expect(dotenvenc.decrypt({ passwd: ENC_PASSWD })).to.be.rejectedWith(/tampered with or corrupted/);
    });
    ...
    it(`should reject a tampered initialization vector`, async () => {
        ...
        iv[0] ^= 0xff;
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, [version, saltText, iv.toString('hex'), authTagText, encText].join(':'));
        await expect(dotenvenc.decrypt({ passwd: ENC_PASSWD })).to.be.rejectedWith(/tampered with or corrupted/);
    });
    ...
    it(`should reject the legacy unauthenticated dotenvenc <= 5.x file format with a clear error`, async () => {
        ...
        await expect(dotenvenc.decrypt({ passwd: ENC_PASSWD })).to.be.rejectedWith(/does not start with a format version tag/);
    });

    it(`should reject a non-hex encrypted file with a clear error`, async () => {
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, `${dotenvenc.CURRENT_FORMAT_VERSION}:zzzz:abab:cdcd:efef`);
        await expect(dotenvenc.decrypt({ passwd: ENC_PASSWD })).to.be.rejectedWith(/salt is not valid hex/);
    });

    it(`should reject an encrypted file whose IV is the wrong length`, async () => {
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, `${dotenvenc.CURRENT_FORMAT_VERSION}:${'ab'.repeat(16)}:${'cd'.repeat(8)}:${'ef'.repeat(16)}:${'01'.repeat(32)}`);
    ...
    it(`should reject a tampered salt`, async () => {
        ...
        salt[0] ^= 0xff;
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, [version, salt.toString('hex'), ivText, authTagText, encText].join(':'));
        await expect(dotenvenc.decrypt({ passwd: ENC_PASSWD })).to.be.rejectedWith(/tampered with or corrupted/);
    });
    ...
    it(`should use the whole password rather than truncating it to 32 bytes`, async () => {
        ...
        expect(await dotenvenc.decrypt({ passwd: longPasswd, encryptedFile: CUSTOM_ENCRYPTED_FILE })).to.have.property('ALPHA', 'bar');
        await expect(dotenvenc.decrypt({ passwd: sharedPrefixPasswd, encryptedFile: CUSTOM_ENCRYPTED_FILE })).to.be.rejectedWith(/wrong password/);
        await expect(dotenvenc.decrypt({ passwd: 'A'.repeat(32), encryptedFile: CUSTOM_ENCRYPTED_FILE })).to.be.rejectedWith(/wrong password/);
    });
    ...
    it(`should derive a fresh random salt on every encryption`, async () => {
        ...
        const first = readFileSync(CUSTOM_ENCRYPTED_FILE, 'utf8').split(':');
        await dotenvenc.encrypt({ passwd: ENC_PASSWD, decryptedFile: TEST_SAMPLE_DECRYPTED_FILE, encryptedFile: CUSTOM_ENCRYPTED_FILE, silent: true });
        const second = readFileSync(CUSTOM_ENCRYPTED_FILE, 'utf8').split(':');
        // same password, same plaintext => different salt, different key, different ciphertext
        expect(first[1]).to.not.equal(second[1]);
        expect(first[4]).to.not.equal(second[4]);
        expect(await dotenvenc.decrypt({ passwd: ENC_PASSWD, encryptedFile: CUSTOM_ENCRYPTED_FILE })).to.have.property('ALPHA', 'bar');
    });

    it(`should write the current format version tag`, async () => {
        await dotenvenc.encrypt({ passwd: ENC_PASSWD, decryptedFile: TEST_SAMPLE_DECRYPTED_FILE, encryptedFile: CUSTOM_ENCRYPTED_FILE, silent: true });
        expect(readFileSync(CUSTOM_ENCRYPTED_FILE, 'utf8').split(':')[0]).to.equal(dotenvenc.CURRENT_FORMAT_VERSION);
    });

    it(`should tell the user to upgrade when the file declares a newer format version`, async () => {
        const [, ...rest] = readFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, 'utf8').split(':');
        writeFileSync(dotenvenc.DEFAULT_ENCRYPTED_FILE, ['v999', ...rest].join(':'));
        await expect(dotenvenc.decrypt({ passwd: ENC_PASSWD })).to.be.rejectedWith(/declares format "v999".*Upgrade dotenvenc/);
    });
...
describe('shell export quoting', () => {
    ...
    beforeEach(() => {
        delete process.env.DOTENVENC_PASS;
        writeFileSync(HOSTILE_DECRYPTED_FILE, `${HOSTILE_ENV}\n`);
    });
    ...
    it('should not execute command substitution present in a secret', async () => {
        ...
        expect(out).to.equal('$(id -un)');
        expect(out).to.not.equal(execFileSync('id', ['-un'], { encoding: 'utf8' }).trim());
    });

    it('should skip names that are not valid shell identifiers instead of breaking the eval', async () => {
        writeFileSync(HOSTILE_DECRYPTED_FILE, 'GOOD=fine\nBAD.NAME=nope\nALSO_GOOD=fine2\n');
        await dotenvenc.encrypt({ passwd: ENC_PASSWD, decryptedFile: HOSTILE_DECRYPTED_FILE, encryptedFile: HOSTILE_ENCRYPTED_FILE, silent: true });
        const consoleLogSpy = sinon.spy(console, 'log');
        ...
        sinon.restore();
        expect(emitted).to.deep.equal([`export GOOD='fine';`, `export ALSO_GOOD='fine2';`]);
...
describe('readable digest file', () => {
    ...
    function writeSecrets(secretValue: string): void {
        writeFileSync(READABLE_DECRYPTED_FILE, `GAMMA=1234\nSECRET=${secretValue}\n`);
    }
    ...
    async function generate(passwd = ENC_PASSWD) {
        ...
        return JSON.parse(readFileSync(READABLE_DIGEST_FILE, 'utf8'));
    }
    ...
    it('should not key the digest with the raw password', async () => {
        ...
        const rawPasswordDigest = createHmac('sha256', ENC_PASSWD).update('1234').digest('hex');
        expect(readable.digests.GAMMA).to.not.equal(rawPasswordDigest);
    });
    ...
    it('should keep digests stable across re-encryption so the file stays diffable', async () => {
        ...
        expect(second.digests).to.deep.equal(first.digests);
    });
    ...
    it('should change only the digest of a value that changed', async () => {
        ...
        expect(after.digests.SECRET).to.not.equal(before.digests.SECRET);
        expect(after.digests.GAMMA).to.equal(before.digests.GAMMA);
    });
    ...
    it('should use a fresh random salt when there is no previous digest file', async () => {
        ...
        removeFile(READABLE_DIGEST_FILE);
        const second = await generate();
        expect(second.salt).to.not.equal(first.salt);
        expect(second.digests.GAMMA).to.not.equal(first.digests.GAMMA);
    });

    it('should start a fresh salt if the existing digest file is unusable', async () => {
        writeFileSync(READABLE_DIGEST_FILE, 'not json at all');
        const readable = await generate();
        expect(readable.salt).to.match(/^[0-9a-f]{32}$/);
        expect(readable.version).to.equal(dotenvenc.CURRENT_FORMAT_VERSION);
    });
...
describe('password validation', () => {
    ...
    it('should refuse to encrypt with a password below the minimum length', async () => {
        await expect(dotenvenc.encrypt({ passwd: 'short12' })).to.be.rejectedWith(/at least 8 characters long \(got 7\)/);
    });

    it('should accept a password at exactly the minimum length', async () => {
        await dotenvenc.encrypt({ passwd: '12345678', silent: true });
        expect(await dotenvenc.decrypt({ passwd: '12345678' })).to.have.property('ALPHA', 'bar');
    });
    ...
    it('should not apply the length floor when decrypting, only when encrypting', async () => {
        ...
        await expect(dotenvenc.decrypt({ passwd: 'abc' })).to.be.rejectedWith(/wrong password/);
    });
    ...
    it('should refuse an empty password returned from the prompt when encrypting', async () => {
        ...
        try {
            await expect(dotenvenc.encrypt({ passwd: '' })).to.be.rejectedWith(/refusing to continue with an empty password/);
        } finally {
            revert();
        }
    });
    ...
    it('should refuse an empty password returned from the prompt when decrypting', async () => {
        const revert = dotenvenc.__set__('promptPassword', async () => Promise.resolve(''));
        try {
            await expect(dotenvenc.decrypt({ passwd: '' })).to.be.rejectedWith(/refusing to continue with an empty password/);
        } finally {
            revert();
        }
    });
    ...
    it('should exit with an error when the password prompt is cancelled with Ctrl+C', async () => {
        ...
        const child = spawn(process.execPath, ['-r', 'ts-node/register', 'src/dotenvenc.ts', '-e', '-i', TEST_SAMPLE_DECRYPTED_FILE, '-o', CUSTOM_ENCRYPTED_FILE], { env, stdio: ['pipe', 'pipe', 'pipe'] });
        let stderr = '';
        ...
        expect(exitCode, 'process exited rather than hanging').to.equal(1);
        expect(stderr).to.match(/Password entry cancelled/);
        expect(existsSync(CUSTOM_ENCRYPTED_FILE)).to.equal(false);
    });
});
